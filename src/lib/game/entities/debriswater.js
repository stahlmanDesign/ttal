...
EntityDebriswater = ig.Entity.extend({
	_wmScalable: true,
	_wmDrawBox: true,
	...
	update: function(){		
		if( this.durationTimer.delta() < 0 && this.nextEmit.delta() >= 0 ) {
			...
			ig.game.spawnEntity( EntityDebriswaterParticle, x, y );
		}
	}
});
...
/*
The particles to spawn by the EntityDebriswater. See particle.js for more details.
*/

EntityDebriswaterParticle = EntityParticle.extend({
	lifetime: 8,
	fadetime: 9,
	bounciness: 0.6,
	vel: {x: 60, y: 20},
	
	animSheet: new ig.AnimationSheet( 'media/debriswater.png', 8, 8 ),
